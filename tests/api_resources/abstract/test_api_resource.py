...
class TestAPIResource(object):
    ...
    async def test_retrieve_and_refresh(self, request_mock):
        ...
        res = await self.MyResource.retrieve("foo*", myparam=5)
    ...
    async def test_request_with_special_fields_prefers_explicit(self, request_mock):
        ...
        await self.MyResource._static_request(
            "get",
            "/v1/myresources/foo",
    ...
    async def test_convert_to_stripe_object(self):
        sample = {
            "foo": "bar",
    ...
    async def test_raise_on_incorrect_id_type(self):
        for obj in [None, 1, 3.14, dict(), list(), set(), tuple(), object()]:
            with pytest.raises(stripe.error.InvalidRequestError):
                await self.MyResource.retrieve(obj)

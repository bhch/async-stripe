...
class TestConfiguration(object):
    async def test_is_creatable(self, request_mock):
        resource = await stripe.billing_portal.Configuration.create(
            business_profile={
    ...
    async def test_is_retrievable(self, request_mock):
        resource = await stripe.billing_portal.Configuration.retrieve(
            TEST_RESOURCE_ID
        )
        request_mock.assert_requested(
    ...
    async def test_is_modifiable(self, request_mock):
        resource = await stripe.billing_portal.Configuration.modify(TEST_RESOURCE_ID)
        request_mock.assert_requested(
    ...
    async def test_is_listable(self, request_mock):
        resource = await stripe.billing_portal.Configuration.list()
        request_mock.assert_requested(

...
class TestCardholder(object):
    async def test_is_creatable(self, request_mock):
        resource = await stripe.issuing.Cardholder.create(
            billing={
                "address": {
                    "city": "city",
                    "country": "US",
    ...
    async def test_is_listable(self, request_mock):
        resources = await stripe.issuing.Cardholder.list()
    ...
    async def test_is_modifiable(self, request_mock):
        resource = await stripe.issuing.Cardholder.modify(
    ...
    async def test_is_retrievable(self, request_mock):
        resource = await stripe.issuing.Cardholder.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
    ...
    async def test_is_saveable(self, request_mock):
        ...
        cardholder = await resource.save()
        request_mock.assert_requested(
            "post", "/v1/issuing/cardholders/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe.issuing.Cardholder)

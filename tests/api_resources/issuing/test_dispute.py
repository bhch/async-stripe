...
class TestDispute(object):
    ...
    async def test_is_modifiable(self, request_mock):
        resource = await stripe.issuing.Dispute.modify(TEST_RESOURCE_ID)
    ...
    async def test_is_retrievable(self, request_mock):
        resource = await stripe.issuing.Dispute.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/issuing/disputes/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe.issuing.Dispute)

    async def test_is_submittable(self, request_mock):
        resource = await stripe.issuing.Dispute.submit(TEST_RESOURCE_ID)
        request_mock.assert_requested(

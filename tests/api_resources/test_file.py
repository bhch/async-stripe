...
class TestFile(object):
    ...
    async def test_is_listable(self, request_mock):
        resources = await stripe.File.list()
    ...
    async def test_is_retrievable(self, request_mock):
        resource = await stripe.File.retrieve(TEST_RESOURCE_ID)
    ...
    async def test_is_creatable(self, setup_upload_api_base, request_mock):
        ...
        resource = await stripe.File.create(
    ...
    async def test_create_respects_stripe_version(
        self, setup_upload_api_base, request_mock
    ):
        test_file = tempfile.TemporaryFile()
        await stripe.File.create(
    ...
    async def test_create_respects_api_version(
        self, setup_upload_api_base, request_mock
    ):
        test_file = tempfile.TemporaryFile()
        await stripe.File.create(

...
class TestListObject(object):
    ...
    async def test_list(self, request_mock, list_object):
        ...
        res = await list_object.list(myparam="you", stripe_account="acct_123")
    ...
    async def test_create(self, request_mock, list_object):
        request_mock.stub_request(
            "post", "/my/path", {"object": "charge", "foo": "bar"}
        )

        res = await list_object.create(myparam="eter", stripe_account="acct_123")
    ...
    async def test_create_maintains_list_properties(self, request_mock, list_object):
        # Testing with real requests because our mock makes it impossible to
        # test otherwise
        charge = await stripe.Charge.retrieve("ch_123", api_key="sk_test_custom")
        res = await charge.refunds.create(amount=123)
        request_mock.assert_requested(
            "post", "/v1/charges/ch_123/refunds", {"amount": 123}, None
        )
        assert res.api_key == "sk_test_custom"

    async def test_retrieve(self, request_mock, list_object):
        request_mock.stub_request(
            "get", "/my/path/myid", {"object": "charge", "foo": "bar"}
        )

        res = await list_object.retrieve(
    ...
    async def test_next_page(self, request_mock):
        ...
        next_lo = await lo.next_page()
        assert not next_lo.is_empty
        assert next_lo.data[0].id == 2

    async def test_next_page_with_filters(self, request_mock):
        lo = stripe.ListObject.construct_from(
            {
                "object": "list",
                "data": [{"id": 1}],
    ...
    async def test_next_page_empty_list(self):
        lo = stripe.ListObject.construct_from(
            {
                "object": "list",
                "data": [{"id": 1}],
    ...
    async def test_prev_page(self, request_mock):
        lo = stripe.ListObject.construct_from(
            {
                "object": "list",
                "data": [{"id": 2}],
        ...
        previous_lo = await lo.previous_page()
        assert not previous_lo.is_empty
        assert previous_lo.data[0].id == 1

    async def test_prev_page_with_filters(self, request_mock):
        lo = stripe.ListObject.construct_from(
            {
                "object": "list",
                "data": [{"id": 2}],
    ...
    async def test_serialize_nested_empty_list(self):
        empty = stripe.ListObject.construct_from(
            {"object": "list", "data": []}, "mykey"
        )
...
class TestAutoPaging:
    ...
    async def test_iter_one_page(self, request_mock):
        lo = stripe.ListObject.construct_from(
    ...
    async def test_iter_two_pages(self, request_mock):
        lo = stripe.ListObject.construct_from(
    ...
    async def test_iter_reverse(self, request_mock):
        lo = stripe.ListObject.construct_from(
    ...
    async def test_class_method_two_pages(self, request_mock):
        request_mock.stub_request(
            "get",
            "/v1/charges",
            {
        ...
        seen = [
            item["id"]
            async for item in await stripe.Charge.auto_paging_iter(limit=25, foo="bar")
        ]

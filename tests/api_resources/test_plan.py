...
class TestPlan(object):
    async def test_is_listable(self, request_mock):
        resources = await stripe.Plan.list()
    ...
    async def test_is_retrievable(self, request_mock):
        resource = await stripe.Plan.retrieve(TEST_RESOURCE_ID)
    ...
    async def test_is_creatable(self, request_mock):
        resource = await stripe.Plan.create(
            amount=100,
            currency="usd",
            id="plan_id",
            interval="month",
    ...
    async def test_is_modifiable(self, request_mock):
        resource = await stripe.Plan.modify(
    ...
    async def test_can_delete(self, request_mock):
        resource = await stripe.Plan.delete(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "delete", "/v1/plans/%s" % TEST_RESOURCE_ID
        )
        assert resource.deleted is True
